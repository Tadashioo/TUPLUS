...
(() => {
  ...
  const tr = value => window.TUPLUS_I18N ? window.TUPLUS_I18N.t(value) : value;
  let spanishCurrency = config.currency;
  try { const stored = localStorage.getItem('tuplus-currency'); if (stored === 'PEN' || stored === 'MXN') spanishCurrency = stored; } catch {}
  let activeCurrency = spanishCurrency;
  const currencyPicker = document.getElementById('site-currency');
  function setLocaleCurrency(locale = 'es') {
    if (locale === 'es') {
      activeCurrency = spanishCurrency;
      currencyPicker?.closest('.currency-picker')?.removeAttribute('hidden');
      if (currencyPicker) currencyPicker.value = activeCurrency;
    } else {
      activeCurrency = config.currencyByLocale[locale] || config.currency;
      currencyPicker?.closest('.currency-picker')?.setAttribute('hidden', '');
    }
    if (currencyPicker) currencyPicker.setAttribute('aria-label', locale === 'es' ? 'Moneda' : 'Currency');
    const fxNote = document.getElementById('fx-rate-note');
    if (fxNote) fxNote.textContent = tr('Importes orientativos convertidos con tasas de referencia del 22 de septiembre de 2026.');
    renderQuote();
  }
  const money = value => new Intl.NumberFormat(document.documentElement.lang || config.locale, {style:'currency',currency:activeCurrency}).format(value * (config.penRates[activeCurrency] || 1));
  currencyPicker?.addEventListener('change', () => { spanishCurrency = currencyPicker.value; activeCurrency = spanishCurrency; try { localStorage.setItem('tuplus-currency', spanishCurrency); } catch {} setLocaleCurrency('es'); });
  ...
  function setMenu(open, restoreFocus = false) {
    ...
    menuButton.setAttribute('aria-label', tr(isOpen ? 'Cerrar menú' : 'Abrir menú'));
  ...
  if (Object.hasOwn(serviceNames, interest || '') && document.getElementById('contact-service')) document.getElementById('contact-service').value = tr(serviceNames[interest]);
  ...
  const estimateText = () => {
    ...
    return [tr('ESTIMACIÓN WEB · TUPLUS'), `${tr('Tipo:')} ${tr(config.types[selection.type].name)}`, `${tr('Plan:')} ${tr(config.plans[selection.plan].name)}`, `${tr('Base del plan:')} ${money(totals.base)}`, `${tr('Ajuste por tipo de web:')} ${money(totals.adjustment)}`, `${tr('Páginas adicionales:')} ${selection.extras} × ${money(config.pricePerPage)} = ${money(totals.additional)}`, `${tr('TOTAL ESTIMADO:')} ${money(totals.total)} (${activeCurrency})`, '', tr('Importe orientativo sujeto a validación comercial. Impuestos, dominio, alojamiento web, integraciones y plazos se confirman en la propuesta.')].join('\n');
  };
  ...
  function renderQuote() {
    ...
    const totals = getTotals();
    document.querySelectorAll('[name="quote-type"]').forEach(input => { const amount = config.types[input.value].price; input.closest('.option-card').querySelector('.choice-price').textContent = amount === 0 ? tr('Base del plan') : (amount > 0 ? '+ ' : '− ') + money(Math.abs(amount)); });
    document.querySelectorAll('[name="quote-plan"]').forEach(input => { input.closest('.plan-option').querySelector('strong').textContent = money(config.plans[input.value].price); });
    document.querySelector('.extras-block p').innerHTML = money(config.pricePerPage) + ' <span>por página</span> · <span>Hasta</span> ' + config.maxExtraPages + ' <span>adicionales</span>';
    ...
    document.getElementById('summary-pages').innerHTML = selection.extras ? `${selection.extras} <span>páginas adicionales</span>` : 'Sin páginas adicionales';
  ...
  if (totalOutput) {
    ...
    renderQuote();
    document.addEventListener('tuplus:language-change', event => setLocaleCurrency(event.detail.locale));
  setLocaleCurrency(window.TUPLUS_I18N?.locale || 'es');
  }
  ...
  ['quote-form', 'contact-form'].forEach(id => {
    ...
    form.addEventListener('submit', event => {
      ...
      if (!quote.validContact(values.contact)) {
        contact.setCustomValidity(tr('Escribe un correo válido o un teléfono de 7 a 15 dígitos con código de país.'));
        contact.reportValidity(); return;
      }
      if (!form.reportValidity()) return;
      const source = id === 'quote-form' ? 'Web TUPLUS / Cotizador' : 'Web TUPLUS / Contacto';
      const message = [`${tr('Hola, soy')} ${values.name}.`, `${tr('Empresa o actividad:')} ${values.company}`, `${tr('Contacto:')} ${values.contact}`, `${tr('País:')} ${values.country}`, `${tr('Preferencia de contacto:')} ${tr(values.preference)}`, '', id === 'quote-form' ? estimateText() : `${tr('Solución de interés:')} ${tr(values.service)}`, values.message ? `\n${tr('Mi proyecto:')} ${values.message}` : '', `\n${tr('Origen:')} ${tr(source)}`, tr('Consentimiento: autorizo el uso de estos datos para atender esta consulta.')].filter(Boolean).join('\n');
      try {
        const result = form.querySelector('.form-result');
